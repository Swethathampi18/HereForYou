...
import { Heart, LayoutDashboard, Users, UserCircle, CalendarDays, ClipboardList, FileCheck, ArrowRightCircle, Settings, LogOut, Eye, Calendar, ArrowRight, X, Loader2 } from "lucide-react";
...
const nav = [
  ...
  { label: "My Patients", icon: UserCircle, to: "/dashboard/therapist" },
  { label: "My Groups", icon: Users, to: "/dashboard/therapist" },
  { label: "Schedule", icon: CalendarDays, to: "/dashboard/therapist" },
  { label: "Session Notes", icon: ClipboardList, to: "/therapist/notes" },
  { label: "Claims", icon: FileCheck, to: "/therapist/claims" },
  { label: "Referrals", icon: ArrowRightCircle, to: "/dashboard/therapist" },
...
const TherapistDashboard = () => {
  ...
  const [stats, setStats] = useState({ patients: 0, groups: 0, sessionsWeek: 0, pendingClaims: 0 });
  ...
  const [ungroupedPatients, setUngroupedPatients] = useState<any[]>([]);
  ...
  const fetchAll = async () => {
    ...
    // Stats
    const [patientCount, groupCount, sessionsWeek, pendingClaims] = await Promise.all([
      ...
      supabase.from("sessions_log").select("id", { count: "exact", head: true }).eq("therapist_id", user.id)
        .in("claims_status", ["pending", "flagged"]),
    ]);
    ...
    // Patients with intake info
    const { data: matchData } = await supabase
      .from("matches")
      .select("user_id, match_type, group_id")
    ...
    if (matchData && matchData.length > 0) {
      ...
      const patientsJoined = (profilesData || []).map((p: any) => {
        ...
        return {
          ...
          lastSession: lastSess ? new Date(lastSess.session_date).toLocaleDateString() : "None",
          intake,
        };
      });
    ...
    const { data: refData } = await supabase
      ...
      .select("*")
      .eq("referring_therapist_id", user.id)
  ...
  const openPatientReport = async (patient: any) => {
    ...
    const { data: moods } = await supabase
      ...
      .limit(30);
    setPatientMoodData((moods || []).map((m: any) => ({
  ...
  const submitSchedule = async () => {
    ...
    const { error } = await supabase.from("sessions_log").insert({
      user_id: schedPatientId,
      therapist_id: user.id,
      session_date: sessionDate.toISOString(),
      notes_text: schedNotes || "",
    ...
    if (error) {
      ...
      await supabase.from("notifications").insert({
        recipient_id: schedPatientId,
        sender_id: user.id,
        type: "appointment",
        message: `New appointment scheduled for ${sessionDate.toLocaleDateString()}`,
      } as any);
  ...
  const submitReferral = async () => {
    ...
    const { error } = await supabase.from("referrals" as any).insert({
      patient_id: refPatientId,
      referring_therapist_id: user.id,
      referred_to: refTo,
  ...
  const openAddPatient = async (groupId: string) => {
    ...
    if (data) {
      ...
      setUngroupedPatients(profiles || []);
    }
    setAddPatientModalOpen(true);
  };

  const submitAddPatient = async () => {
    if (!addPatientId || !selectedGroupId) return;
    setSubmittingAdd(true);
    const { error } = await supabase
      .from("matches")
      .update({ group_id: selectedGroupId, match_type: "group" as const } as any)
      .eq("user_id", addPatientId)
      .eq("therapist_id", user!.id);
    if (error) {
      toast({ title: error.message, variant: "destructive" });
    } else {
      const patientName = ungroupedPatients.find(p => p.id === addPatientId)?.full_name || "Patient";
      const groupName = groups.find(g => g.id === selectedGroupId)?.name || "group";
      toast({ title: `${patientName} added to ${groupName}.` });
  ...
  return (
    <div className="flex min-h-screen bg-background">
          ...
          <div className="p-6 md:p-8 max-w-5xl">
            ...
            <div className="bg-card rounded-xl shadow-sm border mb-8 overflow-hidden">
              <div className="p-5 border-b flex items-center justify-between">
                <h2 className="font-semibold text-foreground">My Patients</h2>
                <div className="flex gap-2">
                  <Button variant="outline" size="sm" onClick={() => setScheduleModalOpen(true)}>Schedule</Button>
              ...
              {loading ? (
                ...
                <div className="p-8 text-center">
                  <p className="text-sm text-muted-foreground">No patients matched yet. Patients will appear here after completing their assessment and being matched to you.</p>
                </div>
              ) : (
                      ...
                      <tr key={p.id} className={i % 2 ? "bg-muted/30" : ""}>
                        ...
                        <td className="p-3">
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                            p.severity === "high" ? "bg-crisis/10 text-crisis" :
                            p.severity === "moderate" ? "bg-warning/10 text-warning" :
                            p.severity === "low" ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"
                          }`}>{p.severity}</span>
                        </td>
                        ...
                        <td className="p-3 text-right">
                          <div className="flex justify-end gap-1">
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => openPatientReport(p)}>
                              <Eye className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setSchedPatientId(p.id); setScheduleModalOpen(true); }}>
                              <Calendar className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setRefPatientId(p.id); setReferralModalOpen(true); }}>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {groups.map((g: any, i: number) => (
                      <tr key={g.id} className={i % 2 ? "bg-muted/30" : ""}>
                        <td className="p-3 font-medium text-foreground">{g.name}</td>
                        <td className="p-3 text-muted-foreground">{g.type?.replace(/_/g, " ")}</td>
                        <td className="p-3 text-muted-foreground">{g.current_count}/{g.max_capacity}</td>
                        <td className="p-3">
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${g.is_active ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}>
                            {g.is_active ? "Active" : "Inactive"}
                          </span>
                        </td>
                        <td className="p-3 text-right">
                          <Button variant="outline" size="sm" onClick={() => openAddPatient(g.id)}>Add Patient</Button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  <tbody>
                    {referrals.map((r: any, i: number) => (
                      <tr key={r.id} className={i % 2 ? "bg-muted/30" : ""}>
                        <td className="p-3 font-medium text-foreground">{r.referred_to}</td>
                        <td className="p-3 text-muted-foreground">{r.specialty}</td>
                        <td className="p-3">
                          <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                            r.urgency === "high" ? "bg-crisis/10 text-crisis" : r.urgency === "medium" ? "bg-warning/10 text-warning" : "bg-muted text-muted-foreground"
                          }`}>{r.urgency}</span>
                        </td>
      ...
      {/* Patient Report Slide-over */}
      <Sheet open={patientReportOpen} onOpenChange={setPatientReportOpen}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          <SheetHeader>
            <SheetTitle>{selectedPatient?.full_name || "Patient Report"}</SheetTitle>
          </SheetHeader>
          {selectedPatient && (
            <div className="space-y-4 mt-4">
              ...
              <div>
                ...
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                  selectedPatient.severity === "high" ? "bg-crisis/10 text-crisis" :
                  selectedPatient.severity === "moderate" ? "bg-warning/10 text-warning" : "bg-success/10 text-success"
                }`}>{selectedPatient.severity}</span>
              </div>
              {selectedPatient.intake && (
                <>
                  ...
                  <div><p className="text-sm text-muted-foreground">Confidence</p><p className="text-sm font-medium">{selectedPatient.intake.confidence_score?.toFixed(2) || "N/A"}</p></div>
      ...
      <Dialog open={scheduleModalOpen} onOpenChange={setScheduleModalOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            <div>
              <Label>Patient</Label>
              <Select value={schedPatientId} onValueChange={setSchedPatientId}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div>
              ...
              <Select value={refSpecialty} onValueChange={setRefSpecialty}>
            ...
            <div><Label>Reason</Label><Textarea className="mt-1" value={refReason} onChange={(e) => setRefReason(e.target.value)} placeholder="Reason for referral" /></div>
            <div>
              <Label>Urgency</Label>
              <RadioGroup value={refUrgency} onValueChange={setRefUrgency} className="flex gap-4 mt-1">
                <div className="flex items-center gap-1"><RadioGroupItem value="low" id="urg-low" /><Label htmlFor="urg-low" className="font-normal">Low</Label></div>
                <div className="flex items-center gap-1"><RadioGroupItem value="medium" id="urg-med" /><Label htmlFor="urg-med" className="font-normal">Medium</Label></div>
                <div className="flex items-center gap-1"><RadioGroupItem value="high" id="urg-high" /><Label htmlFor="urg-high" className="font-normal">High</Label></div>
              </RadioGroup>
            </div>
      ...
      {/* Add Patient to Group Modal */}
      <Dialog open={addPatientModalOpen} onOpenChange={setAddPatientModalOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Add Patient to Group</DialogTitle></DialogHeader>
          <div className="space-y-4 pt-2">
            <div>
              <Label>Patient</Label>
              <Select value={addPatientId} onValueChange={setAddPatientId}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Select patient" /></SelectTrigger>
                <SelectContent>
                  {ungroupedPatients.map((p) => <SelectItem key={p.id} value={p.id}>{p.full_name}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            {ungroupedPatients.length === 0 && <p className="text-sm text-muted-foreground">All patients are already in groups.</p>}
            <Button onClick={submitAddPatient} disabled={submittingAdd || !addPatientId} className="w-full">
              {submittingAdd && <Loader2 className="h-4 w-4 animate-spin mr-2" />}Add to Group
            </Button>
          </div>
        </DialogContent>
      </Dialog>
