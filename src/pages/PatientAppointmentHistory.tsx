...
const statusBadge = (status: string) => {
  const map: Record<string, string> = { attended: "bg-success/10 text-success", missed: "bg-crisis/10 text-crisis", cancelled: "bg-muted text-muted-foreground", rescheduled: "bg-warning/10 text-warning" };
...
const PatientAppointmentHistory = () => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      setLoading(true);
      const { data } = await supabase.from("sessions_log").select("*").eq("user_id", user.id).or(`session_date.lt.${new Date().toISOString()},status.eq.cancelled`).order("session_date", { ascending: false });
      const sess = data || [];
      if (sess.length > 0) {
        ...
        <PageTransition>
          <div className="p-6 md:p-8 max-w-5xl">
            ...
            <div className="bg-card rounded-xl shadow-sm border overflow-hidden">
              {loading ? <Skeleton className="h-64 w-full" /> : sessions.length === 0 ? (
